...
void QDiscordBucket::processLimits()
{
	...
	qDebug()<<this<<"handling limits: "
		   <<"rst:"<<_reset
		  <<"c:"<<static_cast<quint64>(QDateTime::currentSecsSinceEpoch());
#endif
...
#ifdef QDISCORD_LIBRARY_DEBUG
		qDebug()<<this<<"request dequeued, remaining in queue:"
			   <<_queue.length();
#endif
	}
}
...
#ifdef QDISCORD_LIBRARY_DEBUG
	qDebug()<<this<<"request enqueued, remaining in queue:"
		   << _queue.length();
#endif
}
